...
<html>
    <head>
        ...
        <title>Quysy | Quizzes List</title>
    </head>
    <body>
        <div class="container">
            <h1 class="display-5 fw-bold text-center mb-3">Quizzes List</h1>
            <div class="filter-box">
                <form class="filter" action="quizzes-list" method="get">
            ...
            <div class="actions">
                <a href="add-quiz"><button class="btn btn-primary">Add new quiz</button></a>
                <a href="deleted-quizzes-list"><button class="btn btn-danger">Deleted quizzes</button></a>
            </div>
            <div>
                    ...
                    <tbody>
                        <c:forEach var="quiz" items="${quizzes}">
                            <tr>
                                <td>${quiz.quizId}</td>
                                <td>${quiz.quizName}</td>
                                <td><a href="/group6/subject-detail-control?subjectId=${quiz.subjectId}">${quiz.subjectName}</a></td>
                                <td>${quiz.typeName}</td>
                                ...
                                <td>
                                    ...
                                    </c:choose>                                    <form method="post" action="quizzes-list">
                        ...
                        <nav aria-label="Quizzes pagination">
                            <ul class="pagination d-flex justify-content-center">
                                <c:if test="${currentPage > 1}">
                                    <li class="page-item">
                                        <a class="page-link" href="quizzes-list?page=1&searchQuizName=${param.searchQuizName}&subjectName=${param.subjectName}&creatorName=${param.creatorName}&role=${param.role}&level=${param.level}">&lt;&lt;</a>
                                    </li>
                                </c:if>
                                <c:if test="${currentPage > 1}">
                                    <li class="page-item">
                                        <a class="page-link" href="quizzes-list?page=${currentPage - 1}&searchQuizName=${param.searchQuizName}&subjectName=${param.subjectName}&creatorName=${param.creatorName}&role=${param.role}&level=${param.level}">&lt;</a>
                                    </li>
                                </c:if>
                                <c:forEach begin="1" end="${totalPages}" var="i">
                                    <li class="page-item ${currentPage == i ? 'active' : ''}">
                                        <a class="page-link" href="quizzes-list?page=${i}&searchQuizName=${param.searchQuizName}&subjectName=${param.subjectName}&creatorName=${param.creatorName}&role=${param.role}&level=${param.level}">${i}</a>
                                    </li>
                                </c:forEach>
                                ...
                                <c:if test="${currentPage < totalPages}">
                                    <li class="page-item">
                                        <a class="page-link" href="quizzes-list?page=${totalPages}&searchQuizName=${param.searchQuizName}&subjectName=${param.subjectName}&creatorName=${param.creatorName}&role=${param.role}&level=${param.level}">&gt;&gt;</a>
                                    </li>
                                </c:if>
                            </ul>
                        </nav>
