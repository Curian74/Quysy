...
public class QuizzesListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Quiz> allQuizzes = quizDAO.getAvailableQuizzes();
        List<Quiz> quizzes;
        ...
        int totalQuizzes = quizDAO.countFilteredQuizzes(searchQuizName, subjectName, creatorName, role, level);
        int totalPages = (int) Math.ceil((double) totalQuizzes / PAGE_SIZE);

        // Calculate offset for pagination
        int offset = (page - 1) * PAGE_SIZE;

        quizzes = quizDAO.filterQuizzes(searchQuizName, subjectName, creatorName, role, level, offset, PAGE_SIZE);
        ...
        request.getRequestDispatcher("quizzes_list.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("delete".equals(action)) {
            ...
            QuizDAO quizDAO = new QuizDAO();
            quizDAO.deleteQuiz(quizId, updaterId);
        }
